...
// parseChecksums parses a sha256sum-format manifest into name -> hex digest.
// The implementation lives in internal/updatecheck so the tray's self-update
// path verifies artifacts with exactly the same parser.
func parseChecksums(r io.Reader) (map[string]string, error) {
	return updatecheck.ParseChecksums(r)
}

// verifyFileSHA256 hard-fails unless path hashes to wantHex.
func verifyFileSHA256(path, wantHex string) error {
	return updatecheck.VerifyFileSHA256(path, wantHex)
}
