...
- `blocked` counts policy-prevented attempts (persisted as blocked `policy_decision` records, not executed tool_calls). A blocked attempt never ran, so it is **not** included in `calls` and contributes no latency/bytes — it only increments `blocked` and `last_used`. The timeline mirrors the tray glance row-for-row: it counts executed tool calls, blocked attempts (as calls and errors), and mcpproxy's own internal calls (`retrieve_tools`, `describe_tool`, `code_execution`; plus any failed non-management built-in), and excludes `call_tool_*` echo records, management built-ins (`upstream_servers`, `quarantine_security`), and concurrency-rejected sheds. *(Amended by PR #1016 — the timeline previously tracked executed upstream tool_calls only, which left the histogram empty under retrieve_tools/code_execution-heavy traffic; the glance↔timeline parity is pinned by usage_glance_parity_test.go with the Go and Swift admission sets kept in lockstep.)*
