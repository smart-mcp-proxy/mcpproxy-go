...
**Binding-guard delta cases** (FR-008a `BindingGuardDelta`, T033a/T070): these writes do not touch the bound profile or `anonymous_profile` directly, yet each makes a binding bypassable and must be refused with nothing written. Fixture: `require_mcp_auth` off, `anonymous_profile: wf-anon` (a copy of work-full under another name). (a) A client **switchable** on work-readonly (reaches work-readonly + work-full, so `wf-anon` is not wider): `PUT /profiles/work-full` removing `filesystem` from `servers` or adding a deny rule (narrows a member of the binding's `switchable_to`); `PUT /profiles/work-readonly` removing `work-full` from `switchable_to`; `DELETE /profiles/work-full` (removes it from the binding's `switchable_to`; the client itself is not pinned to it, so no `profile_in_use`). (b) A client **locked** to work-full: `DELETE /profiles/work-full?reassign_to=work-readonly` (the binding moves onto a narrower target). (c) Classification: with a client locked to work-readonly and `anonymous_profile` a non-wider copy of it, `profile classify <copy> github:search_code read` (admits the unannotated tool, which work-readonly hides as `unannotated_hidden`). Each is refused identically on REST, CLI and MCP `profiles`.
