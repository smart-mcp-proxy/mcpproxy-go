...
- **FR-074**: Count charts MUST use integer ticks. The usage "unresolved" group MUST be labelled "Calls to unknown tools" on Web. macOS has no usage charts or "unresolved" group today; the same ticks and label apply to the native macOS Usage view when it ships (a follow-up, parity row 22a).
