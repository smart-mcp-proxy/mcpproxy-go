...
- [ ] T006 Write failing tests (after T002 — same file): internal/connect/preview_test.go asserts `ConnectRefusal` is populated for a non-create-capable client (OpenCode) with an absent config using the SAME guard the write uses, and empty otherwise (research D8)
