...
3. Connect one client and let a second client's session go idle (> 5 min old
   activity — can be simulated by editing the seeded sessions' timestamps).
...
| 11 | Summary counts | "N active · M idle" matches rows' states (counting all qualifying, not only displayed) | `01-order.png` |
