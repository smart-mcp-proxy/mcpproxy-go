...
- **C (FR-005)**: `index.Manager.SearchScoped(query, limit, allowedServers)` = conjunction(text query, boost-0 `server_name` disjunction), **proven** score-identical to the FR-005 per-fixture derivation (exhaustive `Search` over the selected corpus, filtered, cut to K) by a test on the 527-tool snapshot; the only permitted fallback is that exhaustive derivation (research D6). Administrators — profiled or not — keep `Search` and today's post-filter byte-for-byte (D3). `total_indexed_tools` uses `server_name` facet term counts filtered by the effective set; `usage_summary.top_tools` filters `GetToolStats(∞)` by scope **and** approval (the exact-tool callability predicate, so indexed-but-pending/changed history is excluded — `mcp_entry_builder_test.go:170-208` shows index presence ≠ approval) then cuts to 10 (admin keeps `GetToolStats(10)` verbatim — the `retrieve_full_stats` golden pins `top_tools:[]`); ordinary retrieve discovery stays quarantine-blind. `session_risk` calls `analyzeSessionRiskScoped(snapshot, serverDiscoverable)`.
...
| Log record (E) | the **existing** zap field `server=<raw name>` at `logger.go:385` — no new field, administrator records unchanged (research D8) | every per-server writer | `ReadUpstreamServerLogTailAttributed(name, n)`: parse the **last** ` | ` segment as the zap fields object (a child message can only precede it), filter by `server` **before** taking the last *n*; lines without a parseable fields object are unattributed → withheld from scoped callers; admin/REST/CLI keep the whole-file reader |
