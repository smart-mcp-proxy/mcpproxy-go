...
**Performance Goals**: FR-011 — scoped `retrieve_tools` within 20 ms p95 of the admin path on the frozen 527-tool snapshot; constitution I (<100 ms BM25 on 1,000 tools) unchanged
...
|---|---|---|
| I. Performance at Scale | PASS with measurement | FR-005 G1 replaces `Search(query, limit)` with a scope-aware search (bleve `server_name` disjunction, oversize-then-filter as fallback). FR-011 in-run delta test (`scope_latency_test.go`) guards the 20 ms bound; exhaustive `Size=documentCount` is rejected (loads 7 stored fields per hit). |
...
| IV. Security by Default | PASS (this is the principle being enforced) | Every change fails closed: no-identity definitions withheld, unresolved target identity refused for scoped callers, legacy/internal cache entries refused, unattributed log lines withheld from scoped callers, foreign containers never removed. Admin fail-open paths kept only where SC-005 names them. |
...
| Performance testing | PASS | FR-011 test + manual merge-base measurement documented in quickstart. |
...
Order is a dependency order, not a preference. Parallel groups share no production files (gap-map §6 collision table).

| # | PR (roadmap task id) | FR / gaps | Why here |
|---|---|---|---|
| **A** | `scope-target-identity-producers` | FR-009 G1–G7 | First: it is a **data-migration event** (raw-name approval records + index docIDs). Every discovery on HEAD deletes exact-name records (`lifecycle.go:783`), so the producer must ship before any reader relies on exact identity. Amends spec text (§ below). |
...
Each PR: failing tests first → fix → `go test -race -count=1 -skip 'E2E|Binary|MCPProtocol|TestInfoEndpoint|TestGracefulShutdownNoPanic|TestSocketInfoEndpoint' ./internal/server/...` + the touched packages → `git diff --stat -- internal/server/testdata` empty (H0 excepted) → golangci-lint v2 → opencode `gpt-6-astra` review (≤10 rounds, verify each finding before fixing) → PR body with "Follow-ups / Spec 105 gaps" checklist naming every gap id it closes.
...
- **C (FR-005)**: `index.Manager.SearchScoped(query, limit, allowedServers)` builds a bleve boolean query `(text) AND server_name ∈ allowed` so BM25 top-K is computed over the authorized population; admins and nil-auth callers keep `Search`. `total_indexed_tools` uses `server_name` facet term counts filtered by the same set; `usage_summary.top_tools` filters `GetToolStats(∞)` by scope **and** index presence, then cuts to 10 (admin keeps `GetToolStats(10)` verbatim — the `retrieve_full_stats` golden pins `top_tools:[]`); `session_risk` calls `analyzeSessionRiskScoped(snapshot, serverDiscoverable)`.
...
| Rendered direct tool (F) | private-struct `_meta` stamp `{owner, rawName, tier}` from the catalog entry that produced it | `renderDirectTools` | both `WithToolFilter`s read the stamp first, consult the catalog only for unstamped entries, **strip the stamp for every caller** (drop the admin early-return) so the `direct_full_prefeature` golden stays byte-exact; built-ins are identified from `builtinDirectToolNames` populated by the constructors, never by parse failure; unstamped non-built-in and empty raw name → withheld from everyone (SC-005 exception) |
| Log record (E) | dedicated unforgeable key `app.mcpproxy/owner=<raw name>` at `logger.go:385` | every per-server writer | `ReadUpstreamServerLogTailAttributed(name, n)`: filter by owner **before** taking the last *n*; pre-key/unattributed lines withheld from scoped callers; admin/REST/CLI keep the whole-file reader |
| Container (E) | existing label `com.mcpproxy.server=<raw>` | `instance.go:57-65` | `ensureNoExistingContainers` + disconnect fallback filter by label (belt-and-braces regex `^mcpproxy-<san>-[a-z0-9]{4}$`); foreign containers are never listed into a's log nor removed |

### 3. Refusals: one constructor per surface, hidden ≡ nonexistent

Pattern from `tailLogNotFound` (`mcp.go:5717-5720`) and `visibleCorpus.notFoundResult` (`preflight/evaluator.go:617-682`). Precedence stays scope → tier → other. Concretely: describe_tool definition mode evaluates existence and case-correction over the **authorized corpus only** (`toolVisibleToSession` reorder in `mcp_visibility.go:51-72`; `mcp_describe_direct.go:150` `continue` instead of `return`); direct catalog keeps a **shadow canonical map** so an authorized canonical `x__y:z` is never withdrawn because a hidden `x` displays `y:z` as `x__y:z`; read_cache collapses unauthorized/not-found/expired into the `cache key not found` body for agent callers (real BBolt errors stay distinct); stored-script not-found no longer enumerates for agent callers. For the direct surface the **scope** check stays inside `WithToolFilter` (mcp-go re-runs it at `tools/call`, which is what makes hidden tools `-32602 not found`) while **tier/callability** move to a list-only path so an over-tier tool on an authorized server reaches the handler and gets the insufficient-permission result.

### 4. Two-fixture differential oracle (H1, consumed by every PR)

`scope_differential_test.go`: `newScopeFixture(t, full bool)` builds either `{a, b, a__b}` (sentinel strings in every hidden name/description/log line/cache payload) or `{a}`; `runScopeScenario(t, name, fn)` runs `fn` against both with an `a`-only token, normalises nondeterministic fields (`normalizeScopeResponse`: ids, timestamps, ranking-dependent fields per SC-001's retrieve-oracle exclusion), and asserts byte equality plus sentinel absence. `scope_http_matrix_test.go` mints real agent tokens (`mintAgentToken(name, allowed, perms, pin)` from D) and drives every `/mcp*` surface × operation over HTTP through `mcpAuthMiddleware`; `n/a` cells assert the unregistered-name `-32602`. `scope_latency_test.go` asserts scoped-vs-admin ≤20 ms on the 527-tool snapshot. PRs A–G each add their scenarios to this harness by US id (SC-007 inventory table lives in H1's file header).
...
4. **FR-009**: note exception (2) applies to administrators too (`spec.md:107`) so the "scoped callers" wording in FR-009 is not read as excluding them from the config-denied `a:ns:erase` rule — while unresolved identity stays fail-open for admins (research.md D4).
...
Re-evaluated after Phase 1: unchanged, all PASS. The only design point that touched a principle was I (performance): `SearchScoped` is a bleve query composition, not an exhaustive scan; the FR-011 test makes the bound executable.
