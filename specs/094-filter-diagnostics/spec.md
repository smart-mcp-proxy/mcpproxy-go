...
- **SC-003**: The serialized `filter_diagnostics` JSON object is at most **600 bytes** in the worst case (all three filters present in the map, all counts at their 3-digit maximum of the 100-cap window, suggestion at its 200-character cap) — enforced by a unit test that serializes a maximal fixture. The shape is fixed, so this bound is exact, not statistical.
