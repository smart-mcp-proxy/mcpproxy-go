...
const handleActivityEvent = (event: CustomEvent) => {
  ...
  const newActivity = event.detail as ActivityRecord
  if (newActivity && newActivity.id) {
    // Check if activity already exists
    const existingIndex = activities.value.findIndex(a => a.id === newActivity.id)
    if (existingIndex >= 0) {
      // Update existing
      activities.value[existingIndex] = newActivity
    } else {
      // Add to beginning
      activities.value.unshift(newActivity)
      // Keep list manageable
      if (activities.value.length > 500) {
        activities.value = activities.value.slice(0, 500)
      }
    }
  }
}
...
const handleActivityCompleted = (event: CustomEvent) => {
  ...
  const completedActivity = event.detail as ActivityRecord
  if (completedActivity && completedActivity.id) {
    const existingIndex = activities.value.findIndex(a => a.id === completedActivity.id)
    if (existingIndex >= 0) {
      activities.value[existingIndex] = completedActivity
    }
    // Also refresh summary
    api.getActivitySummary('24h').then(response => {
