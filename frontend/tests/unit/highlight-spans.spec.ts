...
describe('isSpanUsable', () => {
  ...
  it('accepts an in-range description span whose snippet still checksums', () => {
    const s = span({ start: 9, end: 15, snippet: 'reason' })
    expect(text.slice(9, 15)).toBe('reason')
    expect(isSpanUsable(text, s)).toBe(true)
  })

  it('accepts a span with no snippet at all (offsets are the contract)', () => {
    expect(isSpanUsable(text, span({ start: 9, end: 15 }))).toBe(true)
  })
  ...
  it('rejects a span whose snippet no longer matches the live text', () => {
    ...
    expect(isSpanUsable(text, span({ start: 0, end: 6, snippet: 'reason' }))).toBe(false)
  })
  ...
  it('does NOT relocate a stale span by searching for its snippet', () => {
    ...
    const stale = span({ start: 21, end: 27, snippet: 'reason' })
  ...
  it('compares snippets AFTER applying the same CapEvidence escaping', () => {
    ...
    const s = span({ start: 5, end: 10, snippet: capEvidence('\u200btool') })
    expect(s.snippet).toBe('\\u200btool')
    expect(isSpanUsable(smuggled, s)).toBe(true)
  })

  it('treats a truncated snippet (ending in the ellipsis) as a prefix match', () => {
    const long = 'x'.repeat(400)
    const truncated = capEvidence(long)
    expect(truncated.endsWith('…')).toBe(true)
    expect(isSpanUsable(long, span({ start: 0, end: 400, snippet: truncated }))).toBe(true)
  })

  it('rejects a truncated snippet whose prefix does not match', () => {
    const long = 'y'.repeat(400)
    const truncated = capEvidence('x'.repeat(400))
    expect(isSpanUsable(long, span({ start: 0, end: 400, snippet: truncated }))).toBe(false)
  })

  it('rejects non-description fields (schemas have no render surface in phase 1)', () => {
    expect(isSpanUsable(text, span({ field: 'input_schema', start: 0, end: 3 }))).toBe(false)
    expect(isSpanUsable(text, span({ field: 'output_schema', start: 0, end: 3 }))).toBe(false)
  })

  it('rejects out-of-range, inverted, empty and non-integer offsets', () => {
    expect(isSpanUsable(text, span({ start: -1, end: 4 }))).toBe(false)
    expect(isSpanUsable(text, span({ start: 0, end: text.length + 1 }))).toBe(false)
    expect(isSpanUsable(text, span({ start: 10, end: 4 }))).toBe(false)
    expect(isSpanUsable(text, span({ start: 4, end: 4 }))).toBe(false)
    expect(isSpanUsable(text, span({ start: 0.5, end: 4 }))).toBe(false)
    expect(isSpanUsable(text, span({ start: NaN, end: 4 }))).toBe(false)
  })

  it('accepts a span ending exactly at text.length', () => {
    expect(isSpanUsable(text, span({ start: text.length - 6, end: text.length }))).toBe(true)
  })
  ...
  it('slices UTF-16 code units, so an astral character is spanned correctly', () => {
    ...
    const s = span({ start, end: start + 6, snippet: 'danger' })
...
describe('segmentByFindings — round-trip invariant', () => {
  ...
  it('marks a single interior span and keeps the text whole', () => {
    const s = span({ start: 6, end: 10, snippet: 'beta' })
  ...
  it('handles a span starting at index 0', () => {
    const s = span({ start: 0, end: 5, snippet: 'alpha' })
    const segments = roundTrip(text, [source(s)])
    expect(segments.map((seg) => seg.text)).toEqual(['alpha', ' beta gamma delta'])
  ...
  it('handles a span ending at text.length', () => {
    const s = span({ start: 17, end: text.length, snippet: 'delta' })
    const segments = roundTrip(text, [source(s)])
    expect(segments.map((seg) => seg.text)).toEqual(['alpha beta gamma ', 'delta'])
    expect(segments[1].level).toBe('dangerous')
  })
  ...
  it('handles a span covering the entire description', () => {
    const s = span({ start: 0, end: text.length })
    const segments = roundTrip(text, [source(s)])
  ...
  it('keeps adjacent spans as two separate segments with no lost boundary', () => {
    const a = span({ start: 0, end: 5, snippet: 'alpha', check_id: 'a' })
    const b = span({ start: 5, end: 10, snippet: ' beta', check_id: 'b' })
  ...
  it('collapses an OVERLAP into ONE segment carrying BOTH sources', () => {
    ...
    const a = span({ start: 0, end: 10, check_id: 'tpa.bundle.exfiltration' })
    const b = span({ start: 6, end: 16, check_id: 'shadowing.cross_server' })
    const segments = roundTrip(text, [source(a, 1), source(b, 2)])
    expect(segments.map((seg) => seg.text)).toEqual(['alpha ', 'beta', ' gamma', ' delta'])
  ...
  it('collapses a fully-contained span into the covering one', () => {
    const outer = span({ start: 0, end: 16, check_id: 'outer' })
    const inner = span({ start: 6, end: 10, check_id: 'inner' })
  ...
  it('escalates a mixed-tier overlap to dangerous, and keeps soft-only as warning', () => {
    const soft = span({ start: 0, end: 10, tier: 'soft', check_id: 'soft' })
    const hard = span({ start: 6, end: 10, tier: 'hard', check_id: 'hard' })
  ...
  it('drops unusable spans and still round-trips', () => {
    const good = span({ start: 6, end: 10, snippet: 'beta' })
    const stale = span({ start: 0, end: 5, snippet: 'omega', check_id: 'stale' })
    const outOfRange = span({ start: 100, end: 200, check_id: 'far' })
  ...
  it('round-trips a description carrying zero-width and astral runes', () => {
    ...
    const s = span({ start: 0, end: 7, snippet: capEvidence('ignore\u200b') })
...
describe('usableSources — verification, de-duplication and the mark cap', () => {
  ...
  it('orders marks by position and renumbers them in reading order', () => {
    const later = span({ start: 20, end: 24, check_id: 'later' })
    const earlier = span({ start: 4, end: 8, check_id: 'earlier' })
  ...
  it('de-duplicates identical (start, end, check_id) marks', () => {
    const a = span({ start: 4, end: 8 })
    const b = span({ start: 4, end: 8 })
  ...
  it('caps rendered spans and reports the remainder instead of dropping it silently', () => {
    const many = Array.from({ length: MAX_RENDERED_SPANS + 5 }, (_, i) =>
      source(span({ start: i * 3, end: i * 3 + 2, check_id: `c${i}` }), i + 1),
    )
  ...
  it('excludes unusable spans from both the rendered list and the hidden count', () => {
    const good = source(span({ start: 0, end: 4 }), 1)
    const bad = source(span({ start: 0, end: 4, field: 'input_schema' }), 2)
  ...
  it('counts description spans that no longer verify, separately from the cap', () => {
    const good = source(span({ start: 0, end: 4, snippet: capEvidence(text.slice(0, 4)) }), 1)
    const moved = source(span({ start: 10, end: 14, snippet: 'not-what-is-there' }), 2)
    const schemaOnly = source(span({ start: 0, end: 4, field: 'input_schema' }), 3)
...
describe('excerptSegments — never truncate through a mark', () => {
  ...
  it('never emits a lone surrogate at an excerpt window edge', () => {
    ...
    const marked = source(span({ start: markStart, end: markStart + 6, snippet: undefined }), 1)
  ...
  it('keeps a marked segment whole and elides distant filler', () => {
    ...
    const s = span({ start: 2000, end: 2006, snippet: 'POISON' })
  ...
  it('merges overlapping windows so nearby marks share one excerpt', () => {
    ...
    const first = span({ start: 1000, end: 1002, snippet: 'XX', check_id: 'x' })
    const second = span({ start: 1052, end: 1054, snippet: 'YY', check_id: 'y' })
