...
- **Quarantine**: new servers quarantined until approved; Tool Poisoning Attack (TPA) detection on descriptions. **Tool-level quarantine (Spec 032)**: SHA-256 hashes detect new ("pending") and changed ("changed", rug-pull) tools. Config: `quarantine_enabled` (global), `auto_approve_tool_changes` (per-server; deprecated alias `skip_quarantine`). See [docs/features/security-quarantine.md](docs/features/security-quarantine.md).
