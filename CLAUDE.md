...
4. **Cross-model review round cap — per PR:** when a PR is gated by a cross-model review, run at most **10 fix→re-review rounds on that PR**. If the reviewer has not returned a clean verdict after the 10th round, STOP and ask the human how to proceed (do not auto-run round 11). The counter is per-PR and resets for each new PR. (Verify each finding is genuine before fixing — reviewers do false-positive; a round only counts when you push a fix and re-review.) **The reviewer is the `opencode` CLI, not Codex** — maintainer directive 2026-08-14. Use **`opencode run --model github-copilot/gpt-6-astra "<brief>" < /dev/null`** for both code and spec/plan review (maintainer directive 2026-09-06; `github-copilot/gpt-5.6-sol` is the fallback). Always close stdin and wrap in `gtimeout`, and split the brief into small file-named chunks — opencode exits 0 with no verdict when it is refused a read, so an empty result is not a clean one. The cap was raised from 5 to 10 on 2026-08-31 because round 5 on #1136 caught a real defect the first four missed.
