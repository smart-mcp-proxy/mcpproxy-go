...
4. **Feature-flag adoption matrix** — boolean snapshot of `enable_socket`, `enable_prompts`, `require_mcp_auth`, `enable_code_execution`, `quarantine_enabled`, `sensitive_data_detection_enabled`, plus a sorted/deduped list of OAuth provider *types* (`google` / `github` / `microsoft` / `generic`) derived from upstream URL heuristics.
