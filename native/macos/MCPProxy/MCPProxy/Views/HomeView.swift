...
private struct AttentionRow: View {
    ...
    /// Runs the fix (Spec 109 FR-005). `login`/`restart`/`enable` execute in
    /// place, mirroring the tray's `TrayServerAction.fromHealthAction`. Every
    /// other verb opens the location that performs it — today the server's
    /// detail view (its existing per-tool review for `review`); `/review/<n>`
    /// and `/clients?focus=` get their own native screens in 109-g/109-h.
    private func performFix(_ item: AttentionItem) async {
        switch item.fix.verb {
    ...
    /// other links already use (see the Import/Add Server buttons above) so a
    /// `.showServerDetail` observer set up once in ServersView handles every
    /// doorway into server detail.
    @MainActor
    private func navigateToServerDetail(_ serverName: String, tab: ServerDetailTab) {
        NotificationCenter.default.post(name: .switchToServers, object: nil)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
