...
> **Why the per-record cap?** The age, count and total-size caps all prune the log as a WHOLE; none of them bounds one record. A proxied upstream response can be many megabytes, so a handful of large `call_tool_*` calls could add hundreds of MB to `config.db` with nothing logged to suggest a problem. `activity_max_response_size` cuts the stored text on the write path and marks the record `response_storage_truncated` (a different cut from `response_truncated`, which says the response was cut to `tool_response_limit` on the way to the agent — and whose direction depends on the record type *and* on whether both cuts fired: an `internal_tool_call` logged the FULL text, a `tool_call` logged the agent's own forwarded copy, and a `tool_call` carrying BOTH flags logged strictly less than the agent received. Read `response_truncation_notice` on the record rather than deriving it; `response_bytes` is measured before either cut, so with a forward cut it describes the upstream payload rather than either copy). Setting it to `0` stores responses whole — required for the token benchmark's `--bodies=on-unmasked` measured basis above 64KB, and worth turning off again afterwards. The value is read at startup only.
