...
| Spec-gardener PRs (`claude/spec-gardener`) | The gardener agent runs under the owner's GitHub account, so it cannot approve its own PR. Same-repo branch + a diff confined to `specs/**` and `ROADMAP.md` → `github-actions[bot]` approving review → `gh pr merge --auto --squash`. A gardener PR that touches any other path fails the job and waits for a human. | `.github/workflows/spec-gardener-auto-approve.yml` |
