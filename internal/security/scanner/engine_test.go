...
// TestFilterSyntheticToolsJSONObfuscation verifies that the post-aggregation
// filter drops the well-known MCP-MC-001 false positive on the synthetic
// tools.json metadata file while leaving real findings untouched. tools.json
// is generated by exportToolDefinitions and contains JSON-encoded tool
// descriptions; non-ASCII chars get encoded as \uXXXX escape sequences which
// the obfuscation rule cannot distinguish from deliberate code obfuscation.
func TestFilterSyntheticToolsJSONObfuscation(t *testing.T) {
	cases := []struct {
		name    string
		finding ScanFinding
		dropped bool
		...
		{
			name: "MCP-MC-001 on tools.json with line — DROP",
			finding: ScanFinding{
				RuleID:      "MCP-MC-001",
		...
		{
			name: "MCP-MC-001 on absolute tools.json — DROP",
			finding: ScanFinding{
				RuleID:   "MCP-MC-001",
				Location: "/tmp/mcpproxy-scan-foo-1234/tools.json:1",
			},
			dropped: true,
		},
		{
			name: "MCP-MC-001 on real Python file — KEEP",
			finding: ScanFinding{
				RuleID:   "MCP-MC-001",
		...
		{
			name: "Tool-poisoning rule on tools.json — KEEP (this is exactly why we export it)",
			finding: ScanFinding{
				RuleID:      "MCP-TPA-001",
				Title:       "Hidden instructions in tool description",
				Description: "Tool description contains hidden directives",
				Location:    "tools.json:42",
				Severity:    SeverityHigh,
		...
		{
			name: "Variant obfuscation rule with malicious_code category on tools.json — DROP",
			finding: ScanFinding{
				RuleID:      "PROVIDER-OBF-002",
				Title:       "Encoded payload detected",
				Description: "File contains obfuscated content",
				Category:    "malicious_code",
				Location:    "tools.json:1",
			},
			dropped: true,
		},
		{
			name: "Empty location — KEEP (cannot attribute to tools.json)",
			finding: ScanFinding{
				RuleID:   "MCP-MC-001",
				Location: "",
			},
		...
		{
			name: "tools.jsonl (similar but different) on real file — KEEP",
			finding: ScanFinding{
				RuleID:   "MCP-MC-001",
				Location: "tools.jsonl:1",
			},
			dropped: false,
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := filterSyntheticToolDefinitionFindings([]ScanFinding{tc.finding})
			if tc.dropped {
				if len(got) != 0 {
					t.Errorf("expected finding to be dropped, kept: %+v", got)
				}
			} else {
				if len(got) != 1 {
					t.Errorf("expected finding to be kept, got %d findings", len(got))
				}
			}
		})
	}
...
// TestLocationIsSyntheticToolsJSON guards the path-suffix stripping that lets
// the filter recognise tools.json regardless of how each scanner formats the
// location (bare file, file:line, abs/rel path, with or without column).
func TestLocationIsSyntheticToolsJSON(t *testing.T) {
	cases := []struct {
		location string
		want     bool
	}{
		{"tools.json", true},
		{"tools.json:85", true},
		{"tools.json:1-12", true},
		{"/tmp/foo/tools.json", true},
		{"/tmp/foo/tools.json:42", true},
		{"server.py:42", false},
		{"my-tools.json", false}, // base != "tools.json"
		{"tools.jsonl:1", false},
		{"", false},
	}
	for _, tc := range cases {
		if got := locationIsSyntheticToolsJSON(tc.location); got != tc.want {
			t.Errorf("locationIsSyntheticToolsJSON(%q) = %v, want %v", tc.location, got, tc.want)
		}
	}
}

// TestAggregateReportsDropsObfuscationOnToolsJSON is an integration check
// that the filter is wired through AggregateReports — not just available as
// a standalone helper.
func TestAggregateReportsDropsObfuscationOnToolsJSON(t *testing.T) {
	reports := []*ScanReport{{
		ScannerID: "mcp-ai-scanner",
		Findings: []ScanFinding{
			{RuleID: "MCP-MC-001", Location: "tools.json:85", Severity: SeverityHigh},
			{RuleID: "MCP-TPA-001", Location: "tools.json:10", Severity: SeverityHigh, Category: "tool_poisoning"},
			{RuleID: "MCP-MC-001", Location: "server.py:5", Severity: SeverityHigh},
		},
	}}
	agg := AggregateReports("job1", "test-server", reports)
	for _, f := range agg.Findings {
		if f.RuleID == "MCP-MC-001" && f.Location == "tools.json:85" {
			t.Errorf("expected MCP-MC-001 on tools.json:85 to be filtered, but it survived: %+v", f)
		}
	}
	// Tool-poisoning on tools.json must survive.
	var foundTPA bool
	for _, f := range agg.Findings {
		if f.RuleID == "MCP-TPA-001" {
			foundTPA = true
		}
	}
	if !foundTPA {
		t.Error("MCP-TPA-001 on tools.json should NOT be filtered (tool-poisoning is the whole point)")
	}
	// Real-source MCP-MC-001 must survive.
	var foundReal bool
	for _, f := range agg.Findings {
		if f.RuleID == "MCP-MC-001" && f.Location == "server.py:5" {
			foundReal = true
		}
	}
	if !foundReal {
		t.Error("MCP-MC-001 on real source file should NOT be filtered")
	}
}
