...
func AggregateReports(jobID, serverName string, reports []*ScanReport) *AggregatedReport {
	...
	// Drop false-positive obfuscation findings that fire on the synthetic
	// tools.json file we write into the scan dir. tools.json is produced by
	// json.MarshalIndent of the server's tool definitions; Go's encoder emits
	// non-ASCII characters in tool descriptions as legitimate \uXXXX escape
	// sequences which the AI scanner's MCP-MC-001 rule treats as "obfuscated
	// or encoded payloads". The rule is meaningful for real source code but
	// not for a JSON metadata file we generated ourselves. Tool-poisoning and
	// prompt-injection rules ARE allowed to fire on tools.json — only the
	// obfuscation/encoded-payload rule is suppressed.
	agg.Findings = filterSyntheticToolDefinitionFindings(agg.Findings)
...
// filterSyntheticToolDefinitionFindings removes findings that are known
// false positives generated by content-pattern rules firing on the synthetic
// tools.json file. tools.json is exported by mcpproxy itself (see
// service.go:exportToolDefinitions) so the AI scanner can analyze tool
// descriptions for poisoning/injection. JSON-encoding tool descriptions with
// non-ASCII characters produces \uXXXX escape sequences which look identical
// to a deliberate obfuscation pattern — but the obfuscation rule is meant for
// executable source, not a metadata file we wrote ourselves.
//
// Only obfuscation-style rules are suppressed. Tool-poisoning, prompt-injection,
// and other tool-semantics rules continue to fire on tools.json (this is the
// whole point of writing it for the scanner).
func filterSyntheticToolDefinitionFindings(findings []ScanFinding) []ScanFinding {
	if len(findings) == 0 {
		return findings
	}
	kept := findings[:0]
	for _, f := range findings {
		if isSyntheticToolsJSONObfuscationFalsePositive(&f) {
			continue
		}
		kept = append(kept, f)
	}
...
// isSyntheticToolsJSONObfuscationFalsePositive reports whether a finding is
// a known false positive from running an obfuscation/encoded-payload rule
// against our synthetic tools.json file.
func isSyntheticToolsJSONObfuscationFalsePositive(f *ScanFinding) bool {
	if !locationIsSyntheticToolsJSON(f.Location) {
		return false
	}
	// Match by RuleID (the well-known obfuscation rule from mcp-ai-scanner)
	// or, defensively, by description keywords for variant rules that haven't
	// settled on a stable ID.
	if strings.EqualFold(f.RuleID, "MCP-MC-001") {
		return true
	}
	desc := strings.ToLower(f.Description + " " + f.Title)
	if (strings.Contains(desc, "obfuscat") || strings.Contains(desc, "encoded payload")) &&
		strings.Contains(strings.ToLower(f.Category), "malicious") {
		return true
	}
	return false
}

// locationIsSyntheticToolsJSON reports whether a finding's Location string
// points at the synthetic tools.json metadata file (with or without a line
// number suffix). We deliberately accept "tools.json", "tools.json:85",
// "/abs/path/tools.json:1-12", etc., since different scanners format
// locations differently.
func locationIsSyntheticToolsJSON(location string) bool {
	if location == "" {
		return false
	}
	// Strip any ":<line>[-<col>]" suffix.
	if idx := strings.Index(location, ":"); idx > 0 {
		// Preserve absolute Windows paths (e.g. C:\...): only strip when the
		// remainder starts with a digit.
		if idx+1 < len(location) && location[idx+1] >= '0' && location[idx+1] <= '9' {
			location = location[:idx]
		}
	}
	// filepath.Base handles both / and \ portably.
	return filepath.Base(location) == "tools.json"
}
