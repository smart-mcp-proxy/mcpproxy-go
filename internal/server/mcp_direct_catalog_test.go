...
// TestFilterDirectModeToolsForAuth_UserTypeIsScopeRestricted is the Spec 105
// PR G regression: filterDirectModeToolsForAuth used to key its scope check
// on `authCtx.Type == auth.AuthTypeAgent`, so a server-edition OAuth "user"
// context — which is NOT an administrator (AuthContext.IsAdmin() is false for
// it) but also isn't an agent token — fell through to the unrestricted
// branch and saw every configured server's tools regardless of its own
// AllowedServers. isScopeRestrictedCaller (keyed on IsAdmin()) closes that:
// a "user" token scoped to "we" must not see "hostile"'s tools, exactly like
// an equivalently-scoped agent token.
func TestFilterDirectModeToolsForAuth_UserTypeIsScopeRestricted(t *testing.T) {
	tools := []*config.ToolMetadata{
		{ServerName: "hostile", Name: "steal_secrets", Description: "Should stay hidden", ParamsJSON: `{"type":"object"}`, Hash: "h-steal",
			Annotations: &config.ToolAnnotations{ReadOnlyHint: boolPtr(true)}},
		{ServerName: "we", Name: "solo", Description: "Fine", ParamsJSON: `{"type":"object"}`, Hash: "h-solo",
			Annotations: &config.ToolAnnotations{ReadOnlyHint: boolPtr(true)}},
	}
	...
	rawTools := []mcp.Tool{{Name: "hostile__steal_secrets"}, {Name: "we__solo"}}
	...
	filtered := p.filterDirectModeToolsForAuth(scopedUser, rawTools)
	names := make([]string, 0, len(filtered))
	for _, tool := range filtered {
		names = append(names, tool.Name)
	}
	assert.NotContains(t, names, "hostile__steal_secrets", "a user token scoped to \"we\" must not see \"hostile\"'s tools")
	assert.Contains(t, names, "we__solo", "the user token's own server stays visible")

	// Positive control: an admin_user (the real "operator" identity in the
	// server edition) is unrestricted, same as api-key admin.
	adminUser := auth.WithAuthContext(context.Background(), auth.AdminUserContext("a1", "admin@example.com", "Admin", "google"))
	filtered = p.filterDirectModeToolsForAuth(adminUser, rawTools)
	names = names[:0]
	for _, tool := range filtered {
		names = append(names, tool.Name)
	}
	assert.ElementsMatch(t, []string{"hostile__steal_secrets", "we__solo"}, names, "an OAuth admin user must see every server, like api-key admin")
}
