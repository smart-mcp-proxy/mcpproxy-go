...
func TestCalculateHealth_StatusVocabulary(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "quarantined and user logged out (FR-010)",
			input: HealthCalculatorInput{
				Enabled:       true,
				Quarantined:   true,
				OAuthRequired: true,
		...
		{
			name: "quarantined and OAuth token expired (FR-010)",
			input: HealthCalculatorInput{
				Enabled:       true,
				Quarantined:   true,
				OAuthRequired: true,
		...
		{
			name: "quarantined and OAuth status error while connected (FR-010)",
			input: HealthCalculatorInput{
				Enabled:       true,
				Quarantined:   true,
				OAuthRequired: true,
				OAuthStatus:   "error",
				LastError:     "some non-oauth-shaped error text",
			},
			wantStatus:  StatusSignInRequired,
			wantUsable:  false,
			wantAction:  ActionLogin,
		...
		{
			name: "quarantined and OAuth never authenticated (FR-010)",
			input: HealthCalculatorInput{
				Enabled:       true,
				Quarantined:   true,
				OAuthRequired: true,
