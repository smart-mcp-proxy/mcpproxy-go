...
// NotConnectedError is returned when an oauth_connect upstream has no per-user
// credential yet. It carries the authorize URL the caller must redirect the
// user to in order to connect the upstream (FR-013, actionable error).
...
func (r *CredentialResolver) Resolve(ctx context.Context, userID string, server *config.ServerConfig) (*UpstreamCredential, error) {
	...
	v, err, _ := r.group.Do(flightKey, func() (interface{}, error) {
		return r.acquire(ctx, userID, serverKey, server)
	})
	if err != nil {
		return nil, err
	}
...
func (r *CredentialResolver) acquire(ctx context.Context, userID, serverKey string, server *config.ServerConfig) (*UpstreamCredential, error) {
	...
	case err == nil && cached != nil:
		...
		// Near-expiry or expired: refresh in place. If refresh fails, fall
		// through to a fresh acquisition rather than serving a stale credential.
		if refreshed, rerr := r.refresh(ctx, userID, serverKey, server, cached); rerr == nil {
			return refreshed, nil
		} else {
			r.logger.Warn("near-expiry credential refresh failed; re-acquiring",
				zap.String("server", server.Name), zap.Error(rerr))
		}
	case errors.Is(err, ErrNotFound):
		// No cache: fall through to acquisition.
	case err != nil:
		// Unexpected store error (not "missing"): surface it.
		return nil, fmt.Errorf("credential resolver: load cached credential: %w", err)
	}

	// 2 / 3. Acquire fresh based on mode.
	switch cfg.Mode {
	...
	case config.AuthBrokerModeOAuthConnect:
		// 3. Configured but the user has not connected: return an actionable
		// error carrying the connect URL.
		...
		if aerr != nil {
			return nil, fmt.Errorf("credential resolver: build connect URL: %w", aerr)
		}
		return nil, &NotConnectedError{ServerName: server.Name, ConnectURL: authURL}
	default:
...
// refresh renews a near-expiry cached credential according to the upstream's
// mode: token-exchange upstreams re-exchange the IdP subject token; connect-flow
// upstreams use the stored refresh token.
func (r *CredentialResolver) refresh(ctx context.Context, userID, serverKey string, server *config.ServerConfig, cached *UpstreamCredential) (*UpstreamCredential, error) {
	cfg := server.AuthBroker
	switch cfg.Mode {
	case config.AuthBrokerModeTokenExchange, config.AuthBrokerModeEntraOBO:
		if r.exchanger == nil {
	...
	case config.AuthBrokerModeOAuthConnect:
		if cached.RefreshToken == "" {
			return nil, fmt.Errorf("credential resolver: connect-flow credential has no refresh token")
		}
		conn, cerr := r.connectorFor(server)
		if cerr != nil {
			return nil, cerr
		}
		return conn.Refresh(ctx, userID)
	default:
		return nil, fmt.Errorf("credential resolver: cannot refresh mode %q", cfg.Mode)
	}
}
