...
import (
	...
	seapi "github.com/smart-mcp-proxy/mcpproxy-go/internal/serveredition/api"
	seauth "github.com/smart-mcp-proxy/mcpproxy-go/internal/serveredition/auth"
...
func setupMultiUserOAuth(deps Dependencies) error {
	...
	sessionManager := seauth.NewSessionManager(userStore, sessionTTL, false) // secure=false for localhost

	// Create OAuth handler
	oauthHandler := seauth.NewOAuthHandler(userStore, sessionManager, cfg, hmacKey, deps.Logger)

	// Create auth middleware
	authMiddleware := seauth.NewServerEditionAuthMiddleware(sessionManager, userStore, cfg, hmacKey, deps.Logger)
	...
	authEndpoints := seapi.NewAuthEndpoints(userStore, sessionManager, cfg, hmacKey, deps.Logger)
	configPath := config.GetConfigPath(deps.Config.DataDir)
	adminHandlers := seapi.NewAdminHandlers(userStore, nil, sessionManager, cfg.AdminEmails, sharedServers, deps.Config, configPath, deps.ManagementService, deps.Logger)
	userHandlers := seapi.NewUserHandlers(userStore, sharedServers, deps.StorageManager, hmacKey, deps.Logger)
	userActivityHandlers := seapi.NewUserActivityHandlers(nil, userStore, sharedServers, deps.Logger)
