...
type AuthEndpoints struct {
	...
	sessionManager      *seauth.SessionManager
...
func NewAuthEndpoints(
	...
	return &AuthEndpoints{
		...
		serverEditionConfig: serverEditionConfig,
		hmacKey:             hmacKey,
		logger:              logger,
	}
}
...
func (h *AuthEndpoints) generateToken(w http.ResponseWriter, r *http.Request) {
	...
	ttl := h.serverEditionConfig.BearerTokenTTL.Duration()
	if ttl <= 0 {
		ttl = 24 * time.Hour
	}

	token, err := seauth.GenerateBearerToken(
		h.hmacKey,
		user.ID,
		user.Email,
		user.DisplayName,
